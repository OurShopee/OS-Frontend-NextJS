...
export const GetPlaceOrderapi = createAsyncThunk("fetchGetPlaceOrderdata", async (input_data) => {
    const res = await GetPlaceOrder(input_data);
    return res;
});
export const checkCouponCodeapi = createAsyncThunk("fetchcheckCouponCodedata", async (input_data) => {
        ...
        builder
            ...
            .addCase(GetPlaceOrderapi.fulfilled, (state, action) => {
                ...
                if (action.payload.status === "success") {
                    ...
                    state.selecteddefaultpaymentmethod = action.payload.data.payment_method.find(m => m.selected).id;
                }
            })
            ...
            .addCase(checkCouponCodeapi.fulfilled, (state, action) => {
                if (action.payload.status === "success") {
                    ...
                    state.coupon = {...action.payload.data, coupon: action.payload.coupon};
                } else {
...
export const { setdonationfee, setshowdonation, setselecteddefaultpaymentmethod, setselecteddeafultoption,setpaymentmethodsdata, clearCouponMsg, clearCoupon, removeCoupon } = paymentslice.actions
