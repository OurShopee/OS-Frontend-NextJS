import { cartlist, changeCartQuantity, getWishListsApi, postWishListApi, removeFromCart } from "@/api/cart";
import { createSlice, createAsyncThunk } from "@reduxjs/toolkit";

export const cartlistapi = createAsyncThunk("fetchcartlistdata", async (input_data) => {
...
export const cartlistWithoutLoaderapi = createAsyncThunk("cartlistWithoutLoaderapi", async (input_data) => {
    const res = await cartlist(input_data);
    if (res.status === 204) {
        return { status: 204 };
    }
...
export const changeCartQuantityapi = createAsyncThunk("fetchchangequantitydata", async (input_data) => {
    const res = await changeCartQuantity(input_data);
    return res;
});

export const removeFromCartapi = createAsyncThunk("fetchremovecartdata", async (input_data) => {
    const res = await removeFromCart(input_data);
    return res;
});

export const getWishLists = createAsyncThunk("getWishLists", async (input_data) => {
    const res = await getWishListsApi(input_data);
    return res.data.data;
});

export const postWishList = createAsyncThunk("postWishList", async (input_data) => {
